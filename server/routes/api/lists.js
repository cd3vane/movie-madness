...
router.delete('/list/:list_id/:movie_id', auth, async (req, res) => {
  try {
    ...
    const movie = list.movies.find((movie) => movie.movieId === req.params.movie_id)
    ...
    const removeIndex = list.movies.map((movie) => movie.movieId).indexOf(req.params.movie_id)
...
router.delete('/:list_id', auth, async (req, res) => {
  try {
    const list = await Lists.findById({ _id: req.params.list_id }).populate('user', 'name')
    ...
    if (list.name === 'Watchlist' || list.name === 'Watched' || list.name === 'Liked') {
